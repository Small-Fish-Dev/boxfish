...
#pragma warning restore SB3000 // Hotloading not supported
		...
		public async override Task<Dictionary<Vector3Int, BaseVoxelVolume<T, U>.Chunk>> Parse( byte[] rawData, Importer importer )
		{
			if ( importer.ColorImporter == null )
			{
				Logger.Warning( $"VoxFormat requires a color importer to be added in the Importer builder struct." );
				return null;
			}
			...
			foreach ( var (model, transformIndex) in targetModels )
			{
				...
				for ( int i = 0; i < length; i++ )
				{
					...
					var imported = importer.ColorImporter( color );

					SetVoxel( voxel.x, voxel.y, voxel.z, imported );
				}
			}
